...
class ColormapTorch:
    ...
    def get_indexes_from_colors(self, colors: torch.Tensor):
        ...
        return torch.linalg.norm(colors[None, :].permute(1, 3, 4, 0, 2) - self.colors, dim=-1).argmin(dim=-1,
...
if __name__ == "__main__":
    ...
    test_array_values_torch = torch.cat([torch.from_numpy(test_array_values)[None, None, :],
                                         torch.from_numpy(generate_random_array_nxn(4))[None, None, :]])
    ...
    batch = torch.cat(
        [torch.from_numpy(generate_random_array_nxn(array_size))[None, None, :] for i in range(batch_size)])
