...
def train(config=None):
    with wandb.init(project="UNET", entity="snikiel", config=config):
        ...
        training_set = SpectrogramsDataset(common_config.dataset_location,
                                           ...
                                           augmentor=GaussianNoise(common_config.noise_mean,
                                                                   common_config.noise_variance),
        ...
        # LR scheduler
        scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, 'min', verbose=False, patience=2, min_lr=0.0,
                                                               factor=0.1)
        ...
        if common_config.present_data:
            ...
            wandb.log({"training_examples": [wandb.Image(image_data[0], caption=image_data[1]) for image_data in
                                             training_examples]})
            print()
            validation_examples = predict_example(model, validation_set, desc="Validation set example")
            wandb.log({"validation_examples": [wandb.Image(image_data[0], caption=image_data[1]) for image_data in
                                               validation_examples]})
